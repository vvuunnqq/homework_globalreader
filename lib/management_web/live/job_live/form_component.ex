defmodule ManagementWeb.JobLive.FormComponent do
  ...
  def update(%{job: job} = assigns, socket) do
    changeset = DevicesAndJobs.change_job(job)
  ...
  def handle_event("validate", %{"job" => job_params}, socket) do
    changeset =
      socket.assigns.job
      |> DevicesAndJobs.change_job(job_params)
  ...
  defp save_job(socket, :edit, job_params) do
    case DevicesAndJobs.update_job(socket.assigns.job, job_params) do
      {:ok, _job} ->
        {:noreply,
         socket
  ...
  defp save_job(socket, :new, job_params) do
    case DevicesAndJobs.create_job(job_params) do
      {:ok, _job} ->
        {:noreply,
         socket
